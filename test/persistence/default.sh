...
check "saves, links, and detaches Claude login state" bash -c "set -e; export HOME=/tmp/persistence-test-home PERSIST_ROOT=/tmp/persistence-test-volume; mkdir -p \"\$HOME/.claude\"; printf '%s' local-token > \"\$HOME/.claude/.credentials.json\"; persistence-login save claude; [ ! -L \"\$HOME/.claude/.credentials.json\" ]; persistence-login restore claude; [ -L \"\$HOME/.claude/.credentials.json\" ]; [ \"\$(readlink \"\$HOME/.claude/.credentials.json\")\" = \"\$PERSIST_ROOT/claude/.credentials.json\" ]; printf '%s' shared-token > \"\$PERSIST_ROOT/claude/.credentials.json\"; [ \"\$(cat \"\$HOME/.claude/.credentials.json\")\" = shared-token ]; persistence-login save claude; [ ! -L \"\$HOME/.claude/.credentials.json\" ]; [ \"\$(cat \"\$HOME/.claude/.credentials.json\")\" = shared-token ]"
